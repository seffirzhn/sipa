...
class Template_sms extends MX_Controller
{
	...
	public function __construct()
	{
        ...
        $this->load->model("template_sms_model");
        $this->column   = array(
    ...
    function datagrid(){
        ...
        $response               = $this->template_sms_model->getDatagrid($param);
        echo json_encode($response);
    }

    function deletedata(){
        $this->auth_model->checkModulecrud("acl_delete",$this->module);
        $response               = $this->template_sms_model->deleteData();
        echo json_encode($response);
    }
    ...
    function showform(){
        ...
        $dt                     = $this->template_sms_model->getDatabyid();
        $param["nama"]          = $dt["nama"]?:"";
        $param["pesan"]         = $dt["pesan"]?:"";
        $data["id"]             = ($dt==null) ? "":$id;
        $data['html']           = $this->load->view('template_sms_form',$param,true);
    ...
    function updatedata(){
        ...
        if($response['status']=="success"){
            $response             = $this->template_sms_model->updateData();
        }
        echo json_encode($response);
    }
