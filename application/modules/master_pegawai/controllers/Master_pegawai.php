...
class Master_pegawai extends MX_Controller
{
	...
	public function __construct()
	{
        ...
        $this->column   = array(
                            ...
                            "nama_pegawai"=>array("text"=>"Nama","sorting"=>"true","type_sort"=>"asc","width"=>20,"align"=>"left"),
                            "nip"=>array("text"=>"NIP","sorting"=>"true","width"=>5,"align"=>"left"),
                            "pangkat"=>array("text"=>"Pangkat","sorting"=>"true","width"=>20,"align"=>"left"),
                            "opd"=>array("text"=>"OPD Asal","sorting"=>"false","width"=>15,"align"=>"left"),
        ...
        $this->module   = "master_pegawai";
        $this->configinput = array(
                            array(
                                'field' => 'newnama_pegawai',
                                'label' => 'Nama Pegawai',
                                'rules' => 'trim|required'
                            ),
                            array(
    ...
    function datagrid(){
        ...
        $response               = $this->master_pegawai_model->getDatagrid($param);
        echo json_encode($response);
    }

    function deletedata(){
        $this->auth_model->checkModulecrud("acl_delete",$this->module);
        $response                 = $this->master_pegawai_model->deleteData();
        echo json_encode($response);
    }
    ...
    function showform(){
        ...
        $dt                         = $this->master_pegawai_model->getDatabyid();
        $this->load->helper('file');    
        $param["id_pegawai"]        = $dt["id_pegawai"]?:"";
        $param["nip"]               = $dt["nip"]?:"";
        $param["id_pangkat"]        = $dt["id_pangkat"]?:"";
        $param["id_opd"]            = $dt["id_opd"]?:"";
        $param["listjenis"]         = array(""=>"-- Pilih Jenis Identitas --","NIP"=>"NIP","NIK"=>"NIK","NIDN"=>"NIDN");
        $param["nama_pegawai"]      = $dt["nama_pegawai"]?:"";
        $param["jabatan"]   = $dt["jabatan"]?:"";
        $param["alamat"]            = $dt["alamat"]?:"";
        $param["no_telp"]           = $dt["no_telp"]?:"";
        $param["email"]             = $dt["email"]?:"";
        $param["tempat_lahir"]      = $dt["tempat_lahir"]?:"";
        $param["tgl_lahir"]         = $dt["tgl_lahir"]?:"";
        $param["file"]              = $dt["file"]?:"";
        $param['ext_file']          = get_mime_by_extension(realpath($param["file"]));
        $param["active"]            = isset($dt["is_active"])?$dt["is_active"]:"1";
        $param["listpangkat"]        = $this->global_model->getDatatwodimension("master_pangkat","id_pangkat,concat(nama_pangkat,' (',golongan_ruang,')') as pangkat",null,"id_pangkat","pangkat",array(""=>"---Pilih Pangkat---"));
        $param["listopd"]        = $this->global_model->getDatatwodimension("master_opd","*",null,"id_opd","nama",array(""=>"---Pilih OPD Asal---"));

        $data["id"]                 = ($dt==null) ? "":$id;
        $data['html']               = $this->load->view('master_pegawai_form',$param,true);
    ...
    function createdata(){
        ...
        if($response['status']=="success"){
            ...
            if(!empty($_FILES["newfile"]["name"])){
                makeDirektori("./resources/master_pegawai");
                $filename                = $_FILES["newfile"]["name"];
                $config['upload_path']   = realpath("resources/master_pegawai/");
                ...
                if(!$this->upload->do_upload('newfile')){
                    ...
                    $fileberkas          = "resources/master_pegawai/".$file_info["file_name"];
                }
            }
            $response  = $this->master_pegawai_model->createData($fileberkas);
        }
        echo json_encode($response);
    }
    ...
    function updatedata(){
        ...
        if($response['status']=="success"){
            ...
            $response             = $this->master_pegawai_model->updateData($fileberkas);
        }
        echo json_encode($response);
    }
