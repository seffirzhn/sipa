...
class Master_pansel_model extends CI_Model {
	private $table="master_pansel";
    private $primarykey="id_pansel";
    ...
    public function getDatagrid($param){
        ...
        $this->db->select("mp.*,case when mp.is_active='1' then '<span class=\"text-success\"><i class=\"ti-check\"></i> Aktif</span>' else '<span class=\"text-danger\"><i class=\"ti-close\"></i> Tidak Aktif</span>' end as active")
    ...
    public function createData($file){        
        $data=array(
                "nama_pansel"=>trim($this->input->post("newnama_pansel")),
                "jenis_identitas"=>trim($this->input->post("newjenis_identitas")),
                "nomor_identitas"=>trim($this->input->post("newnomor_identitas")),
                "alamat"=>trim($this->input->post("newalamat")),
                "no_telp"=>trim($this->input->post("newno_telp")),
                "email"=>trim($this->input->post("newemail")),
                "asal"=>trim($this->input->post("newasal")),
    ...
    public function updateData($file){
        ...
        if($save){
            $this->global_model->insertLog("memperbaharui data master pansel (".$data['nama_pansel'].")");
            $response["status"]   = "success";
            $response["message"]  = "Berhasil memperbaharui data master pansel";
            $response["autohide"] = true;    
        }else{
            $response["status"]   = "error";
            $response["message"]  = "Gagal memperbaharui data master pansel";
        }
        return $response;
    }
}