...
class Data_pelanggaran_model extends CI_Model {
	private $table="laporan_pelanggaran";
    private $primarykey="id_pelanggaran";
    ...
    public function getDatagrid($param){
        ...
        $this->db->select("lp.*, mtp1.nama as nama_kepala,mtp2.nama as nama_wakil,mkp.nama_kategori,mp.nama_pelanggaran")
            ->from($this->table." as lp")
            ->join("paslon p","lp.id_paslon=p.id_paslon")
            ->join("master_daerah md","p.id_daerah=md.id_daerah")
            ->join("master_pelanggaran mp","lp.id_pelanggaran=mp.id_pelanggaran")
            ->join("master_kategori_pelanggaran mkp","mp.id_kategori_pelanggaran=mkp.id_kategori_pelanggaran")
            ->join("master_tokoh_politik mtp1","p.id_topol_kepala=mtp1.id_topol")
            ->join("master_tokoh_politik mtp2","p.id_topol_wakil=mtp2.id_topol")
