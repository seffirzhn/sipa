...
class Auth_model extends CI_Model {
	...
	function getUser(){
		...
		$config["uri"]=API_URI."rest/api/loginUser";
		$config["header"] = array("token:D7ypX0Rlg9fSSLr37aogBucOC6QaBIigT9yCb3VJ");
		$config["post"] = array("username"=>$user,"password"=>$pass);
		$fromapi = $this->global_model->getDatafromservice($config);
		$datauser = json_decode($fromapi);
		...
		if($query->num_rows() > 0){
			...
			if($datauser->loginStatus){
				$nip     = trim($this->input->post("username"));
				$configs["uri"]=API_URI."rest/api/getPegawai";
				$configs["header"] = array("token:D7ypX0Rlg9fSSLr37aogBucOC6QaBIigT9yCb3VJ");
				$configs["post"] = array("nip"=>$nip);
				$hasil = $this->global_model->getDatafromservice($configs);
				$dataapi = json_decode($hasil);
				$datanewuser=array(
                    "user_name"=>$dataapi->datapegawai->nip,
                    "name"=>$dataapi->datapegawai->nama,
                    "password"=>$this->encryption->encrypt($dataapi->datapegawai->nip),
                    "id_opd"=>$dataapi->datapegawai->id_opd,
                    "active"=>1,
                    "created_by"=>1,
                    "created_on"=>date("Y-m-d H:i:s"),
                );
				$save   = $this->db->insert("users",$datanewuser);
            	$id     = $this->db->insert_id();
				if($save){
						$datagroup = array();
						$datagroup = array("user_id"=>$id,"group_id"=>3);
						$this->db->insert("users_groups",$datagroup);
					}
				$query 	= $this->db->select("u.user_id,u.user_name,u.name,u.phone,u.id_daerah,u.id_opd,u.email,u.active as user_active,1 as user_login,u.password, case when u.image_profile<>'' then case when u.extend_image='y' then u.image_profile else concat('".base_url()."',u.image_profile) end else concat('".base_url()."','assets/img/noavatar.png') end as image_profile,u.layout_admin")
								->from("users as u")
								->where("u.user_name",$user)
